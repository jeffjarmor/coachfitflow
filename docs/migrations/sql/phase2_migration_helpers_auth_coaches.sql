-- Phase 2 - helper functions for safe coach migration from Firebase to Supabase
-- Applied on project: bzhqprxolseyxuuwodqs
-- Notes:
-- - SECURITY DEFINER helpers (blocked for anon/authenticated)
-- - Idempotent by email
-- - Writes mapping rows into public.migration_id_map
-- - Includes collision guard (same Supabase user mapped to different Firebase UID)
