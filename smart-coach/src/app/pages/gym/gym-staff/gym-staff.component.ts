...
import { GymCoach, GymCoachRole, isPrimaryGymOwner } from '../../../models/gym-coach.model';
...
export class GymStaffComponent {
  ...
  isPrimaryOwner(member: GymCoach): boolean {
    return isPrimaryGymOwner(this.gym(), member.coachId);
  }

  getRoleLabel(member: GymCoach): string {
    if (this.isPrimaryOwner(member)) return 'Dueño principal';
  ...
  async saveStaff() {
    ...
    if (!member || !gymId || !newRole) return;
    ...
    try {
      ...
      try {
        ...
        this.toastService.error('No se pudo actualizar el rol. El dueño principal no puede modificarse desde Personal.');
        return;
      }
